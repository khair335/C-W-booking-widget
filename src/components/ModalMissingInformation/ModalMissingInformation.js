...
class ModalMissingInformation extends Component {
  ...
  render() {
    ...
    if (currentUserLoaded && isUserAuthorized(currentUser)) {
      if (this.state.showMissingInformationReminder === EMAIL_VERIFICATION || !!currentUser?.id && !currentUser.attributes.emailVerified) {
        content = (
          <EmailReminder
            className={classes}
            user={user}
    ...
    return (
      <Modal
        ...
        isOpen={!!this.state.showMissingInformationReminder || !!currentUser?.id && !currentUser?.attributes.emailVerified}
        onClose={() => {
          this.setState({
            showMissingInformationReminder: null,
