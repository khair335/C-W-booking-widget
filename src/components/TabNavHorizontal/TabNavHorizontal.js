...
const Tab = props => {
  const { className, disabled, text, selected, onClick, linkProps, isDark, isListingPage } = props;
  const darkSkinClasses = isDark
    ? classNames(css.tabContentDarkSkin, {
      ...
      [css.selectedListingPageTab]: isListingPage && selected,
    })
    : null;
...
const TabNavHorizontal = props => {
  const { className, rootClassName, tabRootClassName, tabs, skin, isListingPage } = props;
  ...
  return (
    <nav className={classes}>
      {tabs.map((tab, index) => {
        ...
        return <Tab key={key} className={tabClasses} {...tab} isDark={isDark} isListingPage={isListingPage} />;
      })}
    </nav>
  );
};
 ...
 */
export const LinkTabNavHorizontal = props => null;
