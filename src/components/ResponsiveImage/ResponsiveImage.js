...
import { transformImage } from '../../util/data';
...
const ResponsiveImage = props => {
  ...
  // Gumlet will auto-optimized the image
  if (gumletImage && gumletImage.sourceUrl && gumletImage.key) {
    const { sourceUrl, key, options = {}, styles = {} } = gumletImage;
    return <img
      className={className}
      data-src={transformImage(`${sourceUrl}/${key}`, transformWidth)}
      style={styles}
      {...options}
    />
  };
