  ...
  */
  ...
  useEffect(() => {
    ...
    if (!hasBusinessURL && currentUserId) {
      const pathToProfilePage = (uuid, username) =>
        createResourceLocatorString('ProfilePage', routeConfiguration, { id: uuid, username: (username || '').split(' ').join('-') }, {});
      ...
      const profilePageURL =
        hasMarketplaceRootURL && !rootUrl.includes('localhost')
          ? `${rootUrl}${pathToProfilePage(currentUserId.uuid, currentUserUsername || displayName)}`
          : `https://test-marketplace.com${pathToProfilePage(currentUserId.uuid, currentUserUsername || displayName)}`;
  ...
  return (
    <div className={css.sectionContainer}>
      <H4 as="h3" className={css.accountTypeTitle}>
  ...
  return (
    <FinalForm
      ...
      render={fieldRenderProps => {
        ...
        const { displayName, publicData } = currentUserId ? currentUser.attributes.profile : {};
        const { userName: currentUserUsername } = publicData || {};
