...
export const AvatarComponent = props => {
  ...
  const { banned: isBannedUser, deleted: isDeletedUser, profile } = avatarUser.attributes;
  const { userName } = profile?.publicData || {};
  ...
  const linkProps =
    isUnauthorizedUser && avatarUser.id
      ? {
        ...
        params: { username: userName || displayName, id: avatarUser.id.uuid, variant },
      }
      : avatarUser.id
        ? { name: 'ProfilePage', params: { username: userName || displayName, id: avatarUser.id.uuid } }
  ...
  } else if (hasProfileImage && !profileBanner) {
    return (
      <div {...rootProps}>
        <ResponsiveImage
          rootClassName={css.avatarImage}
  ...
  } else if ((avatarUser.attributes.profile.publicData.profile_banner && isBannerImage) || profileBanner) {
    return (
      <div {...rootProps} className={css.profileBanner}>
        <ResponsiveImage
          rootClassName={css.avatarImage}
          alt={displayName}
          image={avatarUser.attributes.profile.publicData.profile_banner}
          variants={AVATAR_IMAGE_VARIANTS}
          sizes={renderSizes}
        />
      </div>
