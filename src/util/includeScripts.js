...
const gumletSourceSubdomain = process.env.REACT_APP_GUMLET_SOURCE_SUBDOMAIN;
const wasabiDomain = process.env.REACT_APP_WASABI_UNIQUE_DOMAIN;
...
export const IncludeScripts = props => {
  ...
  // <!-- Hotjar Tracking Code for https://www.rabel.co/ --> 
  if (hotjarSiteId && hotjarVersion && process.env.NODE_ENV === 'production') {
    hotjarScript.push(
      <script>
        {`(function(h,o,t,j,a,r){ h.hj=h.hj||function(){(h.hj.q=h.hj.q||[]).push(arguments)}; h._hjSettings={hjid:${hotjarSiteId},hjsv:${hotjarVersion}}; a=o.getElementsByTagName('head')[0]; r=o.createElement('script');r.async=1; r.src=t+h._hjSettings.hjid+j+h._hjSettings.hjsv; a.appendChild(r); })(window,document,'https://static.hotjar.com/c/hotjar-','.js?sv=');`}
      </script>
    )
  };


  if (gumletSourceSubdomain && wasabiDomain) {
    gumletScript.push(
      <script type="text/javascript" key="gumlet">
        {`window.GUMLET_CONFIG = {
          hosts: [{
              current: "${wasabiDomain}",
              gumlet: "${gumletSourceSubdomain}.gumlet.io"
          }],
          lazy_load: true
      };
  ...
  const allScripts = [...analyticsLibraries, ...mapLibraries, ...gumletScript];
