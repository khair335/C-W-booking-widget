...
const sanitizeLabelEnum = (str, options) => (options.map(o => `${o.label}`).includes(str) ? str : null);

const sanitizeMultiEnum = (arr, options) =>
  Array.isArray(arr)
    ? arr.reduce((ret, value) => {
      const enumValue = (typeof value === 'object' && value !== null ? sanitizeLabelEnum(value.label, options) : sanitizeEnum(value, options));
...
export const sanitizeUser = (entity, config = {}) => {
  ...
  const { bio, abbreviatedName, publicData = {}, metadata = {}, ...restProfile } =
    profile || {};
...
const sanitizedExtendedDataFields = (value, config) => {
  ...
  const sanitized =
    schemaType === 'text'
      ? sanitizeText(value)
      : schemaType === 'enum' && typeof value === 'object' && value !== null
      ? sanitizeLabelEnum(value.label, enumOptions)
      : schemaType === 'enum'
