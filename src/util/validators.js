...
export const maxArrayLength = (message, maximumLength) => value => {
  // If no value is provided, return valid
  if (!value) {
    return VALID;
  }
  ...
  // If it's an array, check if its length is within the maximum allowed
  return isArray && value.length <= maximumLength ? VALID : message;
};

export const minimumArrLength = (message, minimumLength) => value => {
...
export const sequenceValidator = (message) => value => {
  if (!value) {
    return message;
  }
  
  // Check if the value is between 1 and 99 (1 or 2 digits)
...
export const userNameValidator = (message, userNameExist) => value => {
  return userNameExist ? message : VALID;
};

export const maxNumber = (message, max) => value => {
  const parsedNumber = parseNum(value);
  return !parsedNumber || (parsedNumber <= max) ? VALID : message;
}

// Regular expression to filter out emojis
const emojiRegex = /[\u{1F600}-\u{1F64F}\u{1F300}-\u{1F5FF}\u{1F680}-\u{1F6FF}\u{1F700}-\u{1F77F}\u{1F780}-\u{1F7FF}\u{1F800}-\u{1F8FF}\u{1F900}-\u{1F9FF}\u{1FA00}-\u{1FAFF}\u{1FB00}-\u{1FBFF}]/u;

export const noEmojiValidator = message => value => {
  // Check if the value contains any emojis using the regular expression
  return emojiRegex.test(value) ? message : VALID;
};

const URL_RE = /^(https?:\/\/)?([a-zA-Z0-9-]+\.)+[a-zA-Z]{2,}(:\d+)?(\/[^\s]*)?$/i;
export const urlFormatValid = message => value => {
  if (!value) return VALID; // url is empty no need to validate
  return value && URL_RE.test(value) ? VALID : message;
};

export const urlPathValidator = message => value => {
  if (!value) return VALID; // url is empty no need to validate
  return value && value.startsWith('/') ? VALID : message;
}

export const subtitleValidator = message => value => {
  if (!value) return VALID; // subtitle is empty no need to validate
  return value && (value.length <= 4 && value.find(v => v.key === 'english')) ? VALID : message;
}
export const validUsernameFormat = message => value => {
  const usernameRegex = /^[a-zA-Z0-9._]*$/; // Allows letters, numbers, ., and _
  return value && usernameRegex.test(value) ? VALID : message;
};
