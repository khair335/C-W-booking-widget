...
import { LoggingAnalyticsHandler, GoogleAnalyticsHandler, KlaviyoAnalyticsHandler, ProductFruitsEventHandler } from './analytics/handlers';
...
const render = (store, shouldHydrate) => {
  ...
  info
    ...
    .then(([_, fetchedAppAssets, currentUser]) => {
      ...
      if (shouldHydrate) {
        ReactDOM.hydrate(
          <ClientApp store={store} hostedTranslations={translations} hostedConfig={hostedConfig} currentUser={currentUser} />,
...
const setupAnalyticsHandlers = (googleAnalyticsId, klaviyoId, productFruitsWorkspaceId) => {
  let handlers = [];
  ...
  if (klaviyoId) {
    handlers.push(new KlaviyoAnalyticsHandler());
  }

  if (productFruitsWorkspaceId) {
    handlers.push(new ProductFruitsEventHandler());
  }
...
if (typeof window !== 'undefined') {
  ...
  // Note: on localhost:3000, you need to use an environment variable
  ...
  const productFruitsWorkspaceId = process.env.REACT_APP_PRODUCT_FRUITS_WORKSPACE_ID;
  const analyticsHandlers = setupAnalyticsHandlers(
    googleAnalyticsId,
    true,
    productFruitsWorkspaceId,
  );
  const store = configureStore(initialState, sdk, analyticsHandlers);

  // Initialize Socket.IO with the Redux store
