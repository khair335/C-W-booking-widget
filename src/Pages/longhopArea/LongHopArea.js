...
import styles from "./LongHopArea.module.css";
...
export default function LongHopArea() {
  ...
  useEffect(() => {
    const fetchPromotions = async () => {
      if (!availablePromotionIds?.length) {
        console.log("No Long Hop promotion IDs available:", availablePromotionIds);
        setIsLoading(false);
        return;
      }
      ...
      try {
        ...
        console.log("Making Long Hop Area API call to:", url);
        console.log("Current pathname:", window.location.pathname);
        console.log("Detected restaurant:", getCurrentRestaurant(pubType, window.location.pathname));

        const response = await getRequest(url, headers);
        console.log("Long Hop Area promotions response:", response.data);

        if (response.data && Array.isArray(response.data)) {
          setPromotions(response.data);
          // Log image URLs for debugging
          response.data.forEach(promo => {
            console.log(`Promotion: ${promo.Name}, Image URL: ${promo.HorizontalImageUrl || 'NO IMAGE'}`);
          });
        } else {
          console.error("Invalid response format from server");
        }
      } catch (error) {
        console.error("Failed to fetch Long Hop Area promotions:", error);
      } finally {
        setIsLoading(false);
      }
    };
