...
export default function Confirmed() {
  ...
  console.log('Confirmed component - Redux state:', {
    specialRequests,
    customerDetails,
    date,
    time,
  ...
  const [comment, setComment] = useState((() => {
    console.log('Confirmed - Initializing comment, specialRequests:', specialRequests);
    ...
    if (!specialRequests.includes('Pre-ordered:')) {
      ...
      console.log('Confirmed - No Pre-ordered, after Session ID removal:', result);
      return result;
    }
    ...
    if (specialRequests.includes(' - Pre-ordered:')) {
      ...
      console.log('Confirmed - With separator, comment part:', commentPart, 'after Session ID removal:', result);
      return result;
      ...
      console.log('Confirmed - Only Pre-ordered content, showing drink info without Session ID:', result);
      return result;
    }

    // Hide Session IDs from any remaining specialRequests content
    const result = specialRequests.replace(/\s*\(Session ID: [^)]+\)/g, '');
    console.log('Confirmed - Fallback, after Session ID removal:', result);
    return result;
  })());
  ...
  useEffect(() => {
    ...
    if (!hasBasicData) {
      console.log('Confirmed page - Missing basic data, attempting to restore from localStorage');
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      console.log('Making Confirmed API call to:', `/api/ConsumerApi/v1/Restaurant/${restaurant}/Booking/${successBookingData.reference}`);
            ...
            {specialRequests && specialRequests.includes('Pre-ordered:') && (() => {
              ...
              if (preOrderedPart) {
                ...
                if (drinkMatch) {
                  ...
                  console.log('✅ ABOUT TO RENDER DRINK SECTION');
                      ...
                      <div className={styles.drinkOrderInfo}>
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            navigate(`/cancel-pre-order-drink?session_id=${sessionId}&source=confirmed`);
                          }}
                        >
                          Cancel
                        </button>
