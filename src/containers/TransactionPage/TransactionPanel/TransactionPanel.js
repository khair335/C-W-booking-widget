...
import { REPORTED_ISSUES } from '../../../constants';
...
export class TransactionPanelComponent extends Component {
  ...
  render() {
    ...
    const { listingType, episodes = [] } = (transactionCollection && transactionCollection.listingData) || {};
    ...
    const anyIssueModal = (
      <Modal
        id="TransactionPanelReportIssueModal"
        isOpen={this.state.reportIssueModal}
        onClose={() => this.setState({ reportIssueModal: false })}
        onManageDisableScrolling={onManageDisableScrolling}
        className={css.reportIssueModal}
      >
        <div>
          <h3 className={css.reportIssueHeading}>
            <FormattedMessage id="TransactionPanel.issueHeading" />
          </h3>
          <p className={css.reportIssueSubHeading}>
            <FormattedMessage id="TransactionPanel.issueSubHeading" />
          </p>
          <div className={css.reportIssuecontent}>
            <label><FormattedMessage id="TransactionPanel.label" /></label>
            <select onChange={(e) => {
              this.setState({ reportedIssue: e.target.value })
            }}>
              <option>Select one</option>
              <option>{REPORTED_ISSUES(intl).unableToWatchMessage}</option>
              <option>{REPORTED_ISSUES(intl).notInLibraryMessage}</option>
            </select>
          </div>

          <Button className={css.requestRefundButton} inProgress={transitionInProgress === transitions.REFUND_REQUEST} onClick={() => {
            const params = {
              protectedData: {
                reportedIssue: this.state.reportedIssue
              }
            }
            onTransition(transaction?.id, transitions.REFUND_REQUEST, params).then(() => {
              this.setState({ reportIssueModal: false })
            })
          }}>
            <FormattedMessage id="TransactionPanel.requestRefund" />
          </Button>
        </div>
          ...
          <div className={css.asideDesktop}>
            <div className={css.stickySection}>
              <div className={css.detailCard}>
                <div className={css.imageWrapperDesktop}>
                  <DetailCardImage
                ...
                {(isCustomer && lastTransition == transitions.AUTO_COMPLETE) ? <div className={css.desktopActionButtons}>
                  <div className={css.actionButtons}>
                    <NamedLink name="ProfilePage" params={{ id: provider.id.uuid, username: provider.attributes.profile.publicData.userName }} className={css.viewMore}><span><FormattedMessage id="TransactionPanel.viewMore" /></span></NamedLink> <br />
                    {!contantWatched ? <span onClick={() => { this.setState({ reportIssueModal: true }) }} className={css.actionButtonText}><FormattedMessage id="TransactionPanel.anyIssue" /></span> : null}
                  </div>
                </div> : null}
