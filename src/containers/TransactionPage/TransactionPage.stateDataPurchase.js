...
export const getStateDataForPurchaseProcess = (txInfo, processInfo) => {
  ...
  const { completeWatched = false } = (transaction && transaction.id && transaction.attributes.metadata) || {};
  ...
  return new ConditionalResolver([processState, transactionRole])
    ...
    .cond([states.COMPLETED, CUSTOMER], () => {
      return {
        ...
        showActionButtons: true,
        primaryButtonProps: completeWatched ? leaveReviewProps : null,
      };
    })
    .cond([states.PAYOUT_CREATED, CUSTOMER], () => {
      return {
        processName,
        processState,
        showDetailCardHeadings: true,
    ...
    .cond([states.REQUEST_TO_REFUND, PROVIDER], () => {
      const actionButtonTranslationId = 'TransactionPage.approveRefund';
      return {
        processName,
        processState,
        showDetailCardHeadings: true,
        showReviewAsFirstLink: true,
        showActionButtons: true,
        primaryButtonProps: actionButtonProps(transitions.APPROVE_REFUND_REQUEST, PROVIDER, {
          actionButtonTranslationId,
        }),
      };
    })
