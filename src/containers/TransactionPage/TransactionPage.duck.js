...
import { getTransactionCollection, singleMailApiCollection, transactionLineItems, updateListingReviews } from '../../util/api';
...
import { ADMIN_EMAIL, ADMIN_NAME, TEMPLATED_ID_FOR_REFUND_REQUEST, TEMPLATED_ID_FOR_REVIEW } from '../../constants';
...
const REVIEW_TX_INCLUDES = ['reviews', 'reviews.author', 'reviews.subject', 'listing'];
...
export const FETCH_TRANSACTION_COLLECTION_REQUEST = 'app/TransactionPage/FETCH_TRANSACTION_COLLECTION_REQUEST';
export const FETCH_TRANSACTION_COLLECTION_SUCCESS = 'app/TransactionPage/FETCH_TRANSACTION_COLLECTION_SUCCESS';
export const FETCH_TRANSACTION_COLLECTION_ERROR = 'app/TransactionPage/FETCH_TRANSACTION_COLLECTION_ERROR';
// ================ Reducer ================ //
...
const initialState = {
  ...
  fetchTransactionCollectionInProgress: false,
  transactionCollection: {},
  fetchTransactionCollectionError: null,
};
...
export default function transactionPageReducer(state = initialState, action = {}) {
  ...
  switch (type) {
    ...
    case FETCH_TRANSACTION_COLLECTION_REQUEST:
      return { ...state, fetchTransactionCollectionInProgress: true, fetchTransactionCollectionError: null };
    case FETCH_TRANSACTION_COLLECTION_SUCCESS:
      return { ...state, fetchTransactionCollectionInProgress: false, transactionCollection: payload };
    case FETCH_TRANSACTION_COLLECTION_ERROR:
      return { ...state, fetchTransactionCollectionInProgress: false, fetchTransactionCollectionError: payload };
...
export const fetchTransactionCollectionRequest = () => ({ type: FETCH_TRANSACTION_COLLECTION_REQUEST });

export const fetchTransactionCollectionSuccess = data => ({
  type: FETCH_TRANSACTION_COLLECTION_SUCCESS,
  payload: data,
});
export const fetchTransactionCollectionError = error => ({
  type: FETCH_TRANSACTION_COLLECTION_ERROR,
  error: true,
  payload: error,
});
// ================ Thunks ================ //
...
export const fetchTransactionCollection = (id, txRole, config) => (dispatch, getState, sdk) => {
  dispatch(fetchTransactionCollectionRequest());
  return getTransactionCollection({
    transactionId: id.uuid,
  }).then((res) => {
    if (res) {
      dispatch(fetchTransactionCollectionSuccess(res.data && res.data[0]));
      return res;
    }
  }).catch(e => {
    dispatch(fetchTransactionCollectionError(storableError(e)));
    throw e;
  });
};
...
export const reviewNotificationToUser = (params) => (dispatch, getState, sdk) => {
  const { txId, customerEmail, customerName } = params || {};
  const payload = {
    transactionUrl: `${process.env.REACT_APP_MARKETPLACE_ROOT_URL}/order/${txId}`,
    email: customerEmail,
    name: customerName,
  };
  singleMailApiCollection({ template_id: TEMPLATED_ID_FOR_REVIEW, data: payload })
    .catch(e => {
      console.log('error', e);
    });
};
...
export const makeTransition = (txId, transitionName, params) => (dispatch, getState, sdk) => {
  ...
  return sdk.transactions
    ...
    .then(response => {
      const { publicData: userPublicData, displayName: name, firstName, lastName } = response.data.included[0].attributes.profile || {};
      const { email, displayName } = userPublicData || {};
      const { publicData, title } = response.data.included[1].attributes || {};
      const { listingType } = publicData || {};
      const transactionId = txId.uuid;
      const { payinTotal, createdAt } = response.data.data.attributes || {};
      dispatch(addMarketplaceEntities(response));
      dispatch(transitionSuccess());
      dispatch(fetchCurrentUserNotifications());
      if (transitionName == transitions.REFUND_REQUEST) {
        const payload = {
          viewerEmail: email,
          viewerName: displayName || name || firstName + ' ' + lastName,
          contentPurchased: { listingType: listingType == FILM_PRODUCTS ? FILM_LABEL : SERIES_LABEL, title, },
          purchaseAmount: payinTotal?.amount / 100,
          purchaseDate: moment(createdAt).format('DD(ddd)-MMM-yyyy'),
          transactionId,
          email: ADMIN_EMAIL,
          name: ADMIN_NAME,
        };
        singleMailApiCollection({ template_id: TEMPLATED_ID_FOR_REFUND_REQUEST, data: payload })
          .catch(e => {
            console.log('error', e);
          })
      }
...
const sendReviewAsFirst = (txId, transition, params, dispatch, sdk, config) => {
  ...
  return sdk.transactions
    ...
    .then(response => {
      ...
      if (response) {
        updateListingReviews({ listingId, rating: params?.reviewRating })
      }
      return response;
    })
    .catch(e => {
...
export const loadData = (params, search, config) => (dispatch, getState) => {
  ...
  return Promise.all([
    ...
    dispatch(fetchTransactionCollection(txId)),
    dispatch(fetchMessages(txId, 1, config)),
