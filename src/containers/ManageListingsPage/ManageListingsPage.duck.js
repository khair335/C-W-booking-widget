...
import { deleteAlgoliaListing, updateAlgoliaData } from '../../util/api';
...
export const closeListing = listingId => (dispatch, getState, sdk) => {
  ...
  return sdk.ownListings
    ...
    .then(response => {
      ...
      // delete from algolia and updated the listing delete variable 
      Promise.all([
        deleteAlgoliaListing({
          indexName: process.env.REACT_APP_ALGOLIA_LISTINGS_INDEX,
          id: listingId.uuid,
        }),
        dispatch(markAsDraft({ id: listingId, publicData: { deletedListing: true } }))
      ]);
      return response;
    })
    .catch(e => {
...
export const deleteDraftListing = listingId => (dispatch, getState, sdk) => {
  dispatch(closeListingRequest(listingId));

  return sdk.ownListings.discardDraft({ id: listingId }, { expand: true })
    .then(response => {
      dispatch(closeListingSuccess(response));
      return response;
    })
...
export const markAsDraft = params => (dispatch, getState, sdk) => {
  dispatch(closeListingRequest(params.id));
  ...
  return sdk.ownListings
    ...
    .then(response => {
      ...
      dispatch(closeListingSuccess(response));
        ...
        updateAlgoliaData({
          indexName: process.env.REACT_APP_ALGOLIA_LISTINGS_INDEX,
          objectID: params.id.uuid,
          markAsDraft: params.publicData.markAsDraft,
        });

      // Need to delete poster url from the user's publicData
      const { marketingPoster } = response.data?.data?.attributes?.publicData || {};

      if (marketingPoster?.url) {
        const state = getState();
        const user = state.user.currentUser;
      
        const { publicData: userPublicData } = user.attributes.profile;
        const { marketingPosters = [] } = userPublicData;
      
        // Determine the updated list of posters
        const isDraft = params.publicData.markAsDraft;
        const isDeleted = params.publicData.deletedListing;
        const updatedMarketingPosters = (isDraft || isDeleted)
          ? marketingPosters.filter((posterUrl) => posterUrl !== marketingPoster.url)
          : [...new Set([...marketingPosters, marketingPoster.url])]; // Ensure no duplicates
      
        // Dispatch the updated profile data
        dispatch(updateProfile({ publicData: { marketingPosters: updatedMarketingPosters } }));
      };
