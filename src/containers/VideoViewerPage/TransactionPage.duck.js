...
import { getTransactionCollection, transactionLineItems } from '../../util/api';
...
import { currentUserShowSuccess, fetchCurrentUserNotifications } from '../../ducks/user.duck';
...
export const updateTheWatchlist = ({ listingId, isWatched }) => (dispatch, getState, sdk) => {
  ...
  .then(response => {
    ...
    const { watchlist = [] } = privateData || {};
    
    // Update the watchlist, exclude the current listing to ensure we don't end up with duplicates
    let updatedWatchList = watchlist.filter(id => id !== listingId);
    
    if (!isWatched) {
      updatedWatchList = [listingId, ...updatedWatchList]; // Add at the start
    }
  
    updatedWatchList = updatedWatchList.slice(0, 10); // Keep only first 10
  
    return sdk.currentUser.updateProfile(
      { privateData: { watchlist: updatedWatchList } },
      { expand: true }
    )
    .then((response) => dispatch(currentUserShowSuccess(response.data.data)))
    .catch(e => {
      log.error(e, 'update-watchlist-failed', {
        listingId: listingId.uuid,
        isWatched,
      });
    });
  ...
  .catch(e => {
    log.error(e, 'fetching-current-user-failed', {
      listingId: listingId.uuid,
      isWatched,
    });
  });
