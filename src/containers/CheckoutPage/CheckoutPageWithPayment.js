...
const handleSubmit = (values, process, props, stripe, submitting, setSubmitting) => {
  ...
  processCheckoutWithPayment(orderParams, requestPaymentParams)
    .then(response => {
      ...
      const orderDetailsPath = pathByRouteName('PaymentCompletedPage', routeConfiguration, {
        id: orderId.uuid,
      });
      const initialValues = {
...
export const CheckoutPageWithPayment = props => {
  ...
  useEffect(() => {
    return () => {
      if (!purchasedRef.current) {
        console.log("=>>>CheckoutPage is unmounting<===");
        dispatch(checkoutAbandoned({ listing: pageData?.listing || {} }));
      }
    };
  }, []);
  ...
  const authorDisplayName = listing?.author?.attributes?.profile?.publicData?.displayName || "";
      ...
      <div className={css.contentContainer}>
        ...
        <div className={css.orderFormContainer}>
          <div className={css.headingContainer}>
            ...
            <div className={classNames(css.priceContainer, css.priceContainerMobile)}>
