...
import { ensureCurrentUser, ensureListing, getUploadKey, isListingAssetsReady } from '../../../util/data';
import {
...
import { ASSET_CATEGORY_FILM, ASSET_EPISODE_THUMBNAIL, ASSET_EPISODE_VIDEO, GUMLET_PROCESSING_STATUS, LISTING_TYPE_FILMS, LISTING_TYPE_SERIES } from '../../../constants';
import { EditListingPreviewPanel } from './EditListingPreviewPanel/EditListingPreviewPanel';
...
const TABS_PRODUCT = [DETAILS, MARKETING, PHOTOS, PRICING, CREDITS, REVIEWS];
...
const SERIES_TABS = [DETAILS, MARKETING, CREDITS, EPISODES, PRICING, REVIEWS];
 ...
 */
const tabLabelAndSubmit = (intl, tab, isNewListingFlow, isPriceDisabled, processName, params) => {
  ...
  if (tab === DETAILS) {
    ...
    submitButtonKey = `EditListingWizard.${processNameString}${newOrEdit}.${params.listingType}.saveDetails`
  } else if (tab === MARKETING) {
    labelKey = 'EditListingWizard.tabLabelMarketing';
    submitButtonKey = `EditListingWizard.${processNameString}${newOrEdit}.${params.listingType}.saveMarketing`;
  } else if (tab === EPISODES) {
    labelKey = 'EditListingWizard.tabLabelEpisodes';
    submitButtonKey = `EditListingWizard.${processNameString}${newOrEdit}.saveEpisodes`;
  } else if (tab === REVIEWS) {
    labelKey = 'EditListingWizard.tabLabelReviews';
    submitButtonKey = `EditListingWizard.${processNameString}${newOrEdit}.saveReviews`;
  }
  else if (tab === PRICING) {
    labelKey = 'EditListingWizard.tabLabelPricing';
    submitButtonKey = `EditListingWizard.${processNameString}${newOrEdit}.${params.listingType}.savePricing`;
    ...
    submitButtonKey = `EditListingWizard.${processNameString}${newOrEdit}.${params.listingType}.savePhotos`;
  } else if (tab === CREDITS) {
    labelKey = 'EditListingWizard.tabLabelCredits';
    submitButtonKey = `EditListingWizard.${processNameString}${newOrEdit}.${params.listingType}.saveCredits`;
  }
 ...
 */
const tabCompleted = (tab, listing, config, uploadQueue, filmGumletAssetInProgress) => {
  const {
    availabilityPlan,
    description,
    geolocation,
  ...
  const uploadFilmsKey = getUploadKey({
    listingId: listing?.id?.uuid,
    assetCategory: ASSET_CATEGORY_FILM,
  });

  const isFilmVideoQueued = !!uploadQueue?.find(task => task.uploadKey === uploadFilmsKey);
  const isFilmVideoInProgress = filmGumletAssetInProgress[listing?.id?.uuid];

  const deliveryOptionPicked = publicData && (shippingEnabled || pickupEnabled);
  const validateEpisodes = (episodes) => {
    // at least three episodes must be uploaded
    const episodeWithIncompleteValues = episodes.slice(0, 3).find(epi => {
      return !epi?.sequenceNumber ||
        !epi.title?.trim() ||
        !epi.description?.trim() ||
        epi[ASSET_EPISODE_VIDEO]?.added === false ||
        epi[ASSET_EPISODE_VIDEO]?.added == 'undefined' ||
        !epi[ASSET_EPISODE_THUMBNAIL]?.key
    });

    return !episodeWithIncompleteValues && episodes.length >= 3;
  }
  ...
  switch (tab) {
    ...
    case PHOTOS:
      return isFilmVideoInProgress || isFilmVideoQueued || filmVideo?.asset_id;
    case CREDITS:
      return !!(makeOptional);
    case MARKETING:
      return !!marketingPoster?.key ||
        !!marketingBanner?.key ||
        !!marketingTrailer?.asset_id;
    case EPISODES:
 ...
 */
const tabsActive = (isNew, listing, tabs, config, uploadQueue, filmGumletAssetInProgress) => {
  return tabs.reduce((acc, tab) => {
    const previousTabIndex = tabs.findIndex(t => t === tab) - 1;
    const validTab = previousTabIndex >= 0;
    const hasListingType = !!listing?.attributes?.publicData?.listingType;
    const prevTabComletedInNewFlow = tabCompleted(tabs[previousTabIndex], listing, config, uploadQueue, filmGumletAssetInProgress);
    const isActive =
...
const createReturnURL = (returnURLType, rootURL, routes, pathParams, listingType) => {
  const path = createResourceLocatorString(
    'EditListingStripeOnboardingPage',
    routes,
    { ...pathParams, returnURLType, listingType },
    {}
  );
...
class EditListingWizard extends Component {
  constructor(props) {
    super(props);
    const listingType = props.params.listingType;
    ...
    this.state = {
      ...
      selectedListingType: props.config.listing.listingTypes.find(config => config.listingType === listingType),
    };
  ...
  render() {
    ...
    const tabs =
      isNewListingFlow && (invalidExistingListingType || !hasListingTypeSelected)
        ...
        : isBookingProcess(processName)
          ...
          : isPurchaseProcess(processName) && params.listingType === LISTING_TYPE_SERIES
            ? tabsForPurchaseProcess(SERIES_TABS, listingTypeConfig)
            : isPurchaseProcess(processName) && params.listingType === LISTING_TYPE_FILMS
    ...
    const tabsStatus = tabsActive(isNewListingFlow, currentListing, tabs, config, uploadQueue, filmGumletAssetInProgress);
    ...
    const listingAssetsStatus = isListingAssetsReady({
      uploadQueue,
      episodeAssets,
      filmGumletAsset,
      marketingAssets,
    ...
    console.log(listingAssetsStatus, 'listingAssetsStatus');

    return (
      <div className={classes}>
        {selectedTab === REVIEWS
          ? <EditListingPreviewPanel
            params={params}
            listing={listing}
            errors={errors}
            ...
            intl={intl}
            updateInProgress={rest.updateInProgress}
            currentUser={currentUser}
            onManageDisableScrolling={onManageDisableScrolling}
            uploadQueue={uploadQueue}
            episodeAssets={episodeAssets}
            filmGumletAsset={filmGumletAsset}
            filmGumletAssetInProgress={filmGumletAssetInProgress}
            ...
            {tabs.map(tab => {
              ...
              return (
                <EditListingWizardTab
                  ...
                  filmGumletAsset={filmGumletAsset}
                  filmGumletAssetInProgress={filmGumletAssetInProgress}
                  filmGumletAssetError={filmGumletAssetError}
                  marketingAssets={marketingAssets}
                  listingAssetsStatus={listingAssetsStatus}
                  episodeAssets={episodeAssets}
