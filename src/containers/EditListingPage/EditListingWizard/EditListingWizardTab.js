...
import EditListingMarketingPanel from './EditListingMarketingPanel/EditListingMarketingPanel';
import css from './EditListingWizardTab.module.css';
import EditListingCreditsPanel from './EditListingCreditsPanel/EditListingCreditsPanel';
import defaultConfig from '../../../config/configDefault';
import EditListingEpisodesPanel from './EditListingEpisodesPanel/EditListingEpisodesPanel';
...
const EditListingWizardTab = props => {
  ...
  const onCompleteEditListingWizardTab = (tab, updateValues, redirect) => {
    ...
    return onUpdateListingOrCreateListingDraft(tab, updateListingValues)
      .then(r => {
        ...
        if (isNewListingFlow && tab !== AVAILABILITY && redirect) {
  ...
  const panelProps = tab => {
    return {
      ...
      onSaveDraft: values => {
        return onCompleteEditListingWizardTab(tab, values, false)
      }
    };
  };
  ...
  switch (tab) {
    ...
    case MARKETING: {
      return (
        <EditListingMarketingPanel
          {...panelProps(MARKETING)}
          config={config}
          history={history}
    ...
    case EPISODES:
      return (<EditListingEpisodesPanel
        {...panelProps(EPISODES)}
    ...
    case PRICING: {
      return (
        <EditListingPricingPanel
          {...panelProps(PRICING)}
          marketplaceCurrency={config.currency}
          listingMinimumPriceSubUnits={defaultConfig.listingMinimumPriceSubUnits}
          config={config}
          history={history}
          listingAssetsStatus={listingAssetsStatus}
        />
