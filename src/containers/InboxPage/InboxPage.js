...
export const InboxItem = props => {
  ...
  const { marketingPoster = {} } = (listing && listing.id && listing.attributes.publicData) || {};
    ...
    <div className={css.item}>
      ...
      >
        <div className={css.itemContent}>
          ...
          <div className={css.itemTitle}>{formatLabel(listing?.attributes?.title)}</div>
          <div className={css.itemDateContainer}>
            <span className={css.itemDate}>{moment(createdAt).format('MMM DD')}</span>
            <span className={css.itemUsernameS}>&nbsp;|&nbsp;{otherUserDisplayName}</span>
          </div>
        </div>
        <div className={css.itemState}>
          ...
          <div className={css.itemPrice}>
            {isCustomer ?
              <span>
                ${payoutTotal.amount / 100} Paid
              </span> :

              <span>
                ${payoutTotal.amount / 100} Earned
              </span>
            }
          </div>
        </div>
...
export const InboxPageComponent = props => {
  ...
  return (
    <Page title={title} scrollingDisabled={scrollingDisabled}>
      {(userType == AUDIENCE_USER_TYPE || tab == 'orders') ? <LayoutSettingsSideNavigation {...navParams}>{data}</LayoutSettingsSideNavigation> : <LayoutSideNavigation {...navParams}>{data}</LayoutSideNavigation>}
    </Page>
  );
};
