...
export default function inboxPageReducer(state = initialState, action = {}) {
  ...
  switch (type) {
    ...
    case FETCH_ORDERS_OR_SALES_SUCCESS: {
      const transactions = sortedTransactions(payload.response.data.data);
      return {
        ...state,
        fetchInProgress: false,
        transactionRefs: entityRefs(transactions),
        pagination: payload.response.data.meta,
        orderLength: payload.transactionLength
      };
    }
...
export const fetchOrdersOrSalesRequest = () => ({ type: FETCH_ORDERS_OR_SALES_REQUEST });
export const fetchOrdersOrSalesSuccess = (response, transactionLength = 0) => ({
  type: FETCH_ORDERS_OR_SALES_SUCCESS,
  payload: { response, transactionLength },
});
