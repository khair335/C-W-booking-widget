...
import { AvatarLarge, NamedLink, InlineTextButton, ResponsiveImage } from '../../../components';
...
const UserCard = props => {
  ...
  const { displayName, userProfileImage, userName } = publicData || {};
  ...
  const links = ensuredUser.id ? (
    <p className={linkClasses}>
      <NamedLink className={css.link} name="ProfilePage" params={{ username: userName || displayName, id: ensuredUser.id.uuid }}>
  ...
  const routeToProfilePage = () => {
    return history.push(
      createResourceLocatorString(
        'ProfilePage',
        routeConfiguration,
        { id: ensuredUser.id.uuid, username: userName || displayName },
        {}
      )
    );
  };
  ...
  return (
    <div className={classes}>
      <div className={css.content}>
        <div onClick={routeToProfilePage}>
        <ResponsiveImage
          gumletImage={{
