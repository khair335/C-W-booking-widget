...
import { Form, LocationAutocompleteInput, OutsideClickHandler } from '../../../../components';
...
const KeywordSearchField = props => {
  ...
  const { keywordSearchWrapperClasses, iconClass, intl, isMobile, inputRef ,setShowDropdownOpen} = props;
  
  const handleCardClick = (linkProps) => {
    setShowDropdown(false);
    history.push(createResourceLocatorString(linkProps.name, routeConfiguration, linkProps.params));
  };

  useEffect(() => {
    setShowDropdownOpen(showDropdown);
  }, [showDropdown]);
    ...
    <div>
      ...
      {showDropdown && (
        <OutsideClickHandler onOutsideClick={() => setShowDropdown(false)}>
          <SearchDropdown onClick={handleCardClick}/>
        </OutsideClickHandler>
      )}
    </div>
  );
...
class TopbarSearchFormComponent extends Component {
  ...
  render() {
    const { onSubmit, appConfig,setShowDropdownOpen, ...restOfProps } = this.props;
