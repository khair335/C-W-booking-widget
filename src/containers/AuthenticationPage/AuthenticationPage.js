...
import { AUDIENCE_USER_TYPE, CREATOR_USER_TYPE, propTypes } from '../../util/types';
...
export const AuthenticationForms = props => {
  ...
  const preselectedUserType = userTypes.find(conf => conf.userType === userType)?.userType || AUDIENCE_USER_TYPE;

  const fromMaybe = from ? { from, to } : null;
  ...
  const handleSubmitSignup = values => {
    const { email, password, fname, lname, displayName, ...rest } = values;
    ...
    const params = {
      ...
      publicData: {
        ...
        email,
        displayName: "", // custom displayName attribute (on this will be used as default)
        ...(userType == CREATOR_USER_TYPE ? { featured: false } : {}),
...
export const AuthenticationPageComponent = props => {
  ...
  const { publicData } = (user && user.id && user.attributes.profile) || {};
  ...
  if (isAuthenticated && from) {
    return (
      <Redirect
        to={{
          pathname: from,
          state: { from, to: location.state?.to },
        }}
      />
    );
  } else if (isAuthenticated && currentUserLoaded && !showEmailVerification) {
    return <NamedRedirect name="LandingPage" />;
  } else if (isAuthenticated && publicData.userType == CREATOR_USER_TYPE) {
    return <NamedRedirect name="LandingPage" />;
  } else if (show404) {
    ...
    >
      ...
      >
        <ResponsiveBackgroundImageContainer
          ...
          useOverlay
        >
          {showEmailVerification && publicData.userType == AUDIENCE_USER_TYPE ? (
