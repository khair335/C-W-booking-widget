...
import { getUserInfo } from '../util/data';
  ...
  return sdk
    ...
    .then(() => {
      ...
      const isLoaded = typeof window !== 'undefined'
        ? window.productFruits?.services?.isLoaded
        : false;
      if (typeof window !== 'undefined' && window.productFruitsInit && !isLoaded) {
        const productFruitsWorkspaceId = process.env.REACT_APP_PRODUCT_FRUITS_WORKSPACE_ID;
        const productFruitsWorkspaceLang = process.env.REACT_APP_PRODUCT_FRUITS_WORKSPACE_LANGUAGE;
        const userInfo = getUserInfo(currentUser);
        if (productFruitsWorkspaceId && productFruitsWorkspaceLang) {
          window.productFruitsInit(
            productFruitsWorkspaceId,
            productFruitsWorkspaceLang,
            userInfo
          );
        }
      }
...
export const logout = () => (dispatch, getState, sdk) => {
  // Destroy Product Fruits
  if (typeof window != 'undefined' && window.productFruits && window.productFruits.services) {
...
export const signup = params => (dispatch, getState, sdk) => {
  ...
  return sdk.currentUser
    .create(params)
    .then(res => {
      dispatch(signupSuccess({ ...params, userId: res.data.data.id.uuid }));
    })
